...
# Convert all reflectance images in database to Adobe RGB color space
for number in tqdm(range(len(imageLists))):
    ...
    ### 1- Reflectance spectra to XYZ under specific illumination, Extended D65 in this example
    ...
    # spectra, cmf_values = t.find_closest_spectra_cmf_illuminant(cube,camera_wvs) # with no interpolation
    illumination = t.setIlluminant(t.closests_wvs, "D65", 0)
    t.normalization_factor(t.closests_wvs, illu, cmf_values, 0)
    ...
    XYZ_img = t.reflectance_to_xyz_v2(interp_spectra,closest_idx,cmf_values,illumination,t.K,rows,cols)
